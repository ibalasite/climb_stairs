...
> 基於 LINE 爬樓梯玩法的 HTML5 線上多人互動抽獎遊戲，支援最多 50 位玩家從不同地點透過瀏覽器加入同一場抽獎活動，主持人掌控揭曉節奏，Mulberry32 PRNG 確保結果 100% 公正可驗證。
...
- **手動逐步揭曉** — 主持人逐步揭曉路徑（US-H04），配合現場節奏製造懸念
- **自動揭曉間隔** — 設定 1–30 秒自動揭曉（US-H05），支援中途切換手動模式
- **一鍵揭曉全部** — 主持人一鍵揭曉所有路徑（US-H06）
- **玩家加入房間** — 輸入房間碼 + 暱稱加入，斷線後 30 秒內可重連復原狀態（US-P05）
- **觀看揭曉動畫** — 路徑行走動畫（桌機 ≥ 30fps，手機 ≥ 24fps），同色標識各玩家（US-P03）
...
詳細設計：[EDD — 工程設計文件](https://ibalasite.github.io/climb_stairs/edd.html) ｜ [ARCH — 架構文件](https://ibalasite.github.io/climb_stairs/arch.html)
...
| `REDIS_PASSWORD` | Redis 密碼（生產環境必填強密碼） | ✅ | `REPLACE_WITH_STRONG_PASSWORD` |
| `JWT_SECRET` | JWT 簽名金鑰（openssl rand -hex 64） | ✅ | `REPLACE_WITH_64_BYTE_HEX_SECRET` |
| `JWT_ALGORITHM` | JWT 演算法 | ✅ | `HS256` |
| `JWT_TTL_SECONDS` | JWT 有效期（秒） | ✅ | `21600` |
| `JWT_CLOCK_SKEW_SECONDS` | JWT 時鐘偏移容忍（秒） | ✅ | `30` |
| `ALLOWED_ORIGINS` | 允許的 CORS 來源（逗號分隔，production 用） | ✅ | `https://your-frontend-domain.com` |
| `ROOM_TTL_SECONDS` | 房間 TTL（秒） | ✅ | `86400` |
| `ROOM_IDLE_EXPIRE_SECONDS` | 閒置房間過期（秒） | ✅ | `300` |
...
| `POST /api/rooms/:code/game/reveal-all` | 一鍵揭曉全部路徑 |
...
| 問題 | 原因 | 解決方式 |
|------|------|---------|
| `REDIS_PASSWORD` 認證失敗 | .env 密碼與 Redis 設定不符 | 確認 .env `REDIS_PASSWORD` 與 `redis.conf` 一致 |
| WebSocket 連線被拒 | JWT 過期或 ALLOWED_ORIGINS 未設定 | 重新加入房間取得新 token；確認 ALLOWED_ORIGINS 含前端 origin |
| Canvas 動畫卡頓 | 瀏覽器硬體加速被停用 | 啟用瀏覽器 GPU 加速；Chrome 設定 → 進階 → 系統 |
| 房間碼無效（404） | 房間 TTL 已過期 | 預設 24 小時 TTL；重新建立房間 |
| K8s Pod 無法拉取映像 | GHCR 權限問題 | repo Settings → Actions → Workflow permissions → Read and write |
| `npm run dev` 無法啟動 | Redis 未啟動或端口被占用 | `redis-server` 或 `docker run -p 6379:6379 redis:7-alpine` |
...
- Docker image push 需 GHCR 套件寫入權限（repo Settings → Actions → Workflow permissions）
...
| PRNG | Mulberry32 + djb2 hash | 確定性、可重播、防預測 |
