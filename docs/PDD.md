...
| 揭曉動畫流暢度 | 無卡頓（≥ 30fps） | Chrome DevTools FPS 量測；**測試條件**：桌機 Chrome（最新版）、50 人滿員房間（設計上限）、一般 WiFi 網路；手機端（Android/iOS mid-range 裝置）FPS 目標待 §16 測試環境確認 |
...
7. **Server 端** 生成本局唯一隨機種子（seed），將 [0..N-1] 以 seed 作為亂數來源隨機打亂後，依 **players 陣列的 joinedAt 由早到晚排序**（joinedAt 相同時以 playerId 字典序升冪作為次要排序鍵，確保排序結果確定性）**依序指派**（即打亂後的第 i 個值指派給 joinedAt 排序第 i 位的玩家）給每位玩家作為 startColumn（此排序與 Lobby 顯示順序一致，確保相同 seed 可完全重現相同的 startColumn 指派結果），並以單一原子操作寫入 seed、rowCount、ladderMap、resultSlots、每位玩家的 startColumn、endColumn **及 result**，**以及 Room.status = `revealing`**（此原子操作將 status 從 `running` 更新為 `revealing`，資料寫入與 status 更新不可分割；player.result 為冗餘儲存欄位，由 endColumn 查找 resultSlots 計算後一併寫入，客戶端直接使用廣播值，不需自行計算；**revealIndex 不在本原子操作寫入範圍內**，進入 `revealing` 後 revealIndex 仍為 0，待 Host 主動觸發後才遞增；seed 不在 `revealing` 前傳送至任何客戶端，待 `finished` 結算頁才可選擇性公開，見 §11.1）
8. 進入揭曉流程
...
- [ ] **PRNG 演算法規格**：需在 PRD 技術規格中指定具體演算法（如 Mulberry32、xoshiro256** 等）、初始化方式及 ①②③ 三步驟間的 PRNG 狀態傳遞方式，確保結果可獨立驗算（影響 §11.1 公平性保證）；**特別地，步驟 ③ 的中獎欄位選取演算法（例如：Fisher-Yates 洗牌取前 W 個消耗 N 次 PRNG，或逐次無放回抽樣消耗 W 次等）需明確指定**，不同選取策略消耗不同數量的 PRNG 值，直接影響跨語言驗算時的序列一致性
...
- [ ] **手機端揭曉動畫 FPS 目標**：§3.3 成功指標表中標注「Android/iOS mid-range 裝置 FPS 目標待 §16 測試環境確認」，具體目標值（≥ 30fps 是否適用手機端，或需調整）待測試環境規格確認後補充
- [x] **再玩一局後 winnerCount 越界**：已解決 → 見 §7.6「離線玩家移除（pruning）後 W 越界處理」規格

---

*文件版本：v2.0（PDD rev 2，對應產品 MVP v1）*  
*最後更新：2026-04-17*  
*狀態：Draft — 待工程評估後確認技術選型*
