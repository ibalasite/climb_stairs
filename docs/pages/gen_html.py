...
def inline_md(text):
    ...
    # images/links first — prevents URL underscores from triggering italic
    text = re.sub(
        r'!\[([^\]]*)\]\(([^)]+)\)',
    ...
    # bold/italic — word-boundary guards on underscore variants
...
def render_page(content, title, banner, pages, current, is_index=False):
    gh = (f'<a class="nav-gh-link" href="{GITHUB_REPO}" target="_blank" rel="noopener">GitHub</a>'
          if GITHUB_REPO else '')
    if is_index:
        bc = f'<a href="index.html">{APP_NAME}</a> &rsaquo; 文件中心'
        if GITHUB_REPO:
            bc += (f' <span style="margin-left:1rem">'
                   f'<a href="{GITHUB_REPO}" target="_blank" rel="noopener" style="color:var(--banner-link)">'
                   f'GitHub &nearr;</a></span>')
    else:
        bc = f'<a href="index.html">{APP_NAME}</a> &rsaquo; {banner}'
...
def health_section():
    ...
    return (
        ...
        '<h2 style="margin-top:0;border-bottom:none">📊 專案健康狀態</h2>'
...
def main():
    ...
    # Build page list
    pages = [('index', '首頁', '🏠')]
    known_order = ['PDD', 'PRD', 'EDD', 'ARCH', 'API', 'SCHEMA', 'DESIGN']
    doc_pages = []
    known_slugs = {'readme'}
