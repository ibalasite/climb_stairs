...
PAGE_META = {
    ...
    'api':       ('API 文件',                '🔌'),
    'schema':    ('Schema 文件',             '🗄️'),
    'design':    ('Client 設計文件',         '🖥️'),
    'test_plan': ('測試計畫 (Test Plan)',     '✅'),
    'bdd':       ('BDD Scenarios',           '🧪'),
}
...
def health_section():
    try:
        d = json.loads((BASE / ".devsop-state.json").read_text())
    ...
    return (
        ...
        f'<div style="font-size:1.5rem;font-weight:700;color:var(--accent)">{completed}/31</div>'
        '</div>'
        '<div style="text-align:center;padding:1rem;background:var(--page-bg);border-radius:6px">'
        '<div style="font-size:0.75rem;color:var(--text-muted);margin-bottom:0.25rem">當前 STEP</div>'
        f'<div style="font-size:1.5rem;font-weight:700;color:var(--accent)">{step}</div>'
        '</div>'
        '</div></section>'
    )
...
def main():
    ...
    # Discover pages
    pages = [('index', '首頁', '🏠')]
    known_order = ['IDEA', 'BRD', 'PRD', 'PDD', 'EDD', 'ARCH', 'API', 'SCHEMA', 'DESIGN', 'TEST_PLAN', 'ALIGN_REPORT']
    doc_pages = []
    ...
    extra_pages = []
    known_slugs = {'readme', 'idea', 'brd', 'prd', 'pdd', 'edd', 'arch', 'api', 'schema', 'design',
                   'test_plan', 'align_report', 'migration_code_snapshot'}
