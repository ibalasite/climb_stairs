...
- **US-H03**：作為主持人，我想要在所有玩家就緒後開始遊戲，以便正式進入揭曉流程。
  ...
  - AC-H03-5：Given N=3 時 rowCount=clamp(9,20,60)=20；N=10 時 rowCount=30；N=21 時 rowCount=clamp(63,20,60)=60，When 遊戲開始後，Then 伺服器廣播的 `rowCount` 與公式計算值完全一致（E2E 自動化驗證三個邊界值）。
...
- **US-H07**：作為主持人，我想要踢除指定玩家，以便移除測試帳號或不當參與者。
  ...
  - AC-H07-5：Given 主持人點擊「再玩一局」，When 新局初始化完成，Then `kickedPlayerIds` 清空（重設為 `[]`），前一局被踢者可使用新暱稱加入新局。
