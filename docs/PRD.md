...
- **US-H03**：作為主持人，我想要在所有玩家就緒後開始遊戲，以便正式進入揭曉流程。
  - AC-H03-1：Given 房間狀態為 `waiting` 且 N ≥ 2 且 W 已設定，When 主持人點擊「開始遊戲」，Then 狀態轉為 `running`，後端生成 seed（UUID hex，以 djb2 hash 轉 uint32）並廣播 `ROOM_STATE`（status=`running`，含 rowCount）；**seed 在 status=`finished` 前禁止傳送給任何客戶端**（防止預測結果）；樓梯結構（ladderMap、resultSlots）於後續 `BEGIN_REVEAL` 時以 seed 原子生成，不在此步驟廣播。
...
- **US-H04**：作為主持人，我想要以手動逐步揭曉每條路徑，以便配合現場節奏製造懸念。
  - AC-H04-1：Given 房間狀態為 `running`，When 主持人點擊「開始揭曉」（觸發 `BEGIN_REVEAL`），Then 伺服器以單一原子操作完成 ladderMap、resultSlots、所有 startColumn/endColumn/result 及 status（`revealing`）的寫入，並廣播 `ROOM_STATE`（status=`revealing`），所有客戶端顯示揭曉控制介面；操作須在 1 秒內完成。
...
- FR-04-4：`running→revealing` 由 Host 傳送 `BEGIN_REVEAL` 訊息觸發，Server 以單一原子操作完成（ladderMap、resultSlots、所有 startColumn/endColumn/result、status 同時寫入），seed 在 status=finished 前禁止傳送給任何客戶端。
