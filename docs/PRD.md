...
**版本**：v1.4  **日期**：2026-04-20  **基於 PDD**：v2.2
...
- **US-H01**：作為主持人，我想要建立一個帶有唯一 6 碼房間碼的房間，以便玩家能快速找到並加入我的抽獎活動。
  - AC-H01-1：Given 主持人點擊「建立房間」，When 系統接受請求，Then 於 2 秒內回傳 6 碼 Room Code（由大寫 A-Z 排除 O/I、數字 2-9 組成，共 32 字元集，長度固定 6 碼；即排除視覺混淆字元 O、I、0、1，數字範圍為 2–9）且在 Redis 中不重複。
  ...
  - AC-H01-4：Given Room Code 生成時碰撞次數超過 10 次，When 系統無法在上限次數內找到唯一碼，Then 回傳 `ROOM_CODE_GENERATION_FAILED` 錯誤，前端顯示「建立失敗，請重試」。

- **US-H02**：作為主持人，我想要設定中獎名額（W），以便控制本局有多少玩家會獲獎。
  - AC-H02-1：Given 房間狀態為 `waiting`，When 主持人在輸入框填入 W（1 ≤ W ≤ N-1，**N = players 陣列全部長度，含 isOnline=false 的斷線玩家**），Then 系統接受並廣播更新後的 `ROOM_STATE` 給所有在線玩家。Host 可在 `waiting` 狀態隨時發送 `UPDATE_WINNER_COUNT` 訊息更新中獎名額。
  - AC-H02-2：Given 主持人輸入 W ≤ 0 或 W ≥ N，When 主持人送出，Then 系統顯示「中獎名額須介於 1 到玩家數減 1 之間」錯誤，不更新狀態；**N 定義同上（含 isOnline=false）**。
...
- **US-H03**：作為主持人，我想要在所有玩家就緒後開始遊戲，以便正式進入揭曉流程。
  - AC-H03-1：Given 房間狀態為 `waiting` 且 N ≥ 2 且 W 已設定，When 主持人點擊「開始遊戲」，Then 狀態轉為 `running`，後端以原子操作同步完成 seed 生成（UUID hex，以 djb2 hash 轉 uint32）、ladderMap 計算與 resultSlots 指派（seed 生成與樓梯計算合一，避免中間狀態），並廣播 `ROOM_STATE`（status=`running`，含 rowCount）；**seed 及完整樓梯資料在 status=`finished` 前禁止傳送給任何客戶端**（防止預測結果）；BEGIN_REVEAL 時伺服器只執行狀態轉換（`running → revealing`）並廣播揭曉開始，不重新生成樓梯。
  - AC-H03-2：Given 主持人點擊「開始遊戲」但 W 尚未設定，When 系統收到請求，Then 拒絕並回傳錯誤「請先設定中獎名額」，狀態維持 `waiting`。
  - AC-H03-3：Given 遊戲開始後，When 任何玩家（含主持人）收到 `ROOM_STATE`，Then 所有客戶端渲染的樓梯結構與橫槓位置完全一致，Canvas 動畫 FPS 桌機（1080p Chrome 最新穩定版）≥ 30、手機（Chrome DevTools Moto G4 throttling profile，10 秒錄製平均值）≥ 24；若任一裝置平均 FPS 低於下限，QA 標記此 AC 為失敗。
  - AC-H03-4：Given 房間只有 Host 一人（N < 2），When 主持人點擊「開始遊戲」，Then 系統拒絕並回傳 `INSUFFICIENT_PLAYERS` 錯誤，前端顯示「人數不足（至少需要 2 位玩家）」，狀態維持 `waiting`。
  - AC-H03-5：`rowCount` 公式為 `clamp(N×3, 20, 60)`（最小 20，最大 60）。Given N=3 時 rowCount=clamp(9,20,60)=20；N=10 時 rowCount=clamp(30,20,60)=30；N=21 時 rowCount=clamp(63,20,60)=60，When 遊戲開始後，Then 伺服器廣播的 `rowCount` 與公式計算值完全一致（E2E 自動化驗證三個邊界值，測試腳本以 `Math.min(60, Math.max(20, N*3))` 計算期望值並與廣播值比對）。

- **US-H04**：作為主持人，我想要以手動逐步揭曉每條路徑，以便配合現場節奏製造懸念。
  - AC-H04-1：Given 房間狀態為 `running`（ladderMap 及 resultSlots 已於 START_GAME 原子生成），When 主持人點擊「開始揭曉」（觸發 `BEGIN_REVEAL`），Then 伺服器執行狀態轉換（`running → revealing`）並廣播 `ROOM_STATE`（status=`revealing`），所有客戶端顯示揭曉控制介面；操作須在 1 秒內完成。
  ...
  - AC-H04-5：Given 狀態為非 `running`（如 `waiting`、`revealing`、`finished`），When 主持人嘗試觸發 `BEGIN_REVEAL`，Then 伺服器拒絕並回傳 `INVALID_STATE` 錯誤，狀態不變。

- **US-H05**：作為主持人，我想要設定自動揭曉間隔（1～30 秒/格），以便讓遊戲在無人操作時自動進行。
  - AC-H05-1：Given 主持人選擇自動模式並設定間隔秒數 T（1 ≤ T ≤ 30，**T 須為正整數**），When 遊戲進入 `revealing`，Then 每隔 T 秒伺服器自動廣播下一個 `REVEAL_INDEX`，直到全部揭曉完畢。
  - AC-H05-2：Given 自動揭曉進行中，When 主持人切換回手動模式，Then 自動計時停止，改由主持人手動觸發後續揭曉，已揭曉項目不受影響。
  - AC-H05-3：Given 主持人輸入 T ≤ 0、T > 30、T 非整數（如 1.5）或 T 為非數字，When 系統驗證，Then 回傳 `INVALID_AUTO_REVEAL_INTERVAL` 錯誤並顯示「間隔須為 1～30 的整數」，不啟動自動揭曉。

- **US-H06**：作為主持人，我想要一鍵揭曉全部路徑，以便在時間緊迫時快速完成抽獎。
  - AC-H06-1：Given 狀態為 `revealing` 且尚有未揭曉路徑，When 主持人點擊「全部揭曉」，Then 伺服器廣播 `REVEAL_ALL`，所有客戶端同步播放剩餘所有路徑動畫（可壓縮時長），2 秒內完成渲染；若超過 2 秒仍未完成，動畫立即跳至終止幀（final frame），所有路徑直接顯示靜態完成狀態。
  - AC-H06-2：Given `REVEAL_ALL` 廣播後，When 伺服器完成全部揭曉，Then 伺服器自動將 `revealedCount` 設為 `totalCount`，並立即觸發 `finished` 狀態轉換，廣播 `GAME_ENDED`（即 `ROOM_STATE { status: finished, seed, results[] }`）；客戶端動畫在 2 秒內完成渲染（若超時則跳至終止幀）。**`REVEAL_ALL_TRIGGER` 自動完成揭曉並廣播 finished，無需主持人再次確認，以提升操作流暢度**。
...
- **US-H07**：作為主持人，我想要踢除指定玩家，以便移除測試帳號或不當參與者。
  ...
  - AC-H07-3：Given 房間狀態為 `running` 或之後，When 主持人嘗試踢除玩家，Then 系統拒絕並回傳 `INVALID_STATE` 錯誤（包含 `KICK_NOT_ALLOWED_IN_STATE` 說明）。
  ...
  - AC-H07-5：Given 主持人點擊「再玩一局」，When 新局初始化完成，Then `kickedPlayerIds` 清空（重設為 `[]`），前一局被踢者**可使用任意暱稱（含原暱稱或新暱稱）以無 playerId 方式（視為新玩家）加入新局**；被踢者的舊 playerId 已失效，需以新連線取得新 playerId。
...
- **US-H08**：作為主持人，我想要在一局結束後發起「再玩一局」，以便用同一房間繼續下一輪抽獎。
  ...
  - AC-H08-3：Given 再玩一局後剔除離線玩家，剩餘在線玩家數 < 2（即在線玩家數為 0 或 1），When 主持人點擊「再玩一局」，Then 系統拒絕並回傳 `INSUFFICIENT_PLAYERS` 錯誤，前端提示「在線玩家不足（至少需要 2 位），無法開始新局」。
...
- **US-H09**：作為主持人，我想要在等待大廳複製邀請連結，以便讓參加者能快速加入房間。
...
- **US-P01**：作為玩家，我想要輸入暱稱並透過 6 碼房間碼加入房間，以便參與抽獎活動。
  - AC-P01-1：Given 玩家輸入有效暱稱（1～20 個 Unicode 字元）及正確 Room Code，When 送出後 WebSocket 握手成功，Then 從送出按鈕點擊起計 1.5 秒內進入等待畫面，玩家列表顯示自己的暱稱（量測：Playwright `page.click()` 至 waiting 元素可見的時間差）。
  - AC-P01-2：Given 玩家輸入的暱稱與同房間已有玩家重複（**暱稱比對區分大小寫，即 "Alice" 與 "alice" 視為不同暱稱**），When 伺服器驗證，Then 回傳 `NICKNAME_TAKEN` 錯誤，提示「此暱稱已被使用，請換一個」。
  - AC-P01-3：Given 玩家輸入空白暱稱或超過 20 字元，When 前端驗證，Then 即時顯示欄位錯誤訊息，送出按鈕保持 disabled；若前端驗證被繞過（如直接 WebSocket 訊息），伺服器端亦拒絕並回傳 `INVALID_NICKNAME` 錯誤（對應 FR-02-2）。
...
- **US-P03**：作為玩家，我想要觀看自己路徑的揭曉動畫，以便體驗抽獎的緊張感。
  - AC-P03-1：Given 輪到自己的路徑揭曉（`REVEAL_INDEX` 指向自己），When 動畫播放，Then Canvas 高亮自己的路徑，動畫 FPS 桌機（1080p Chrome 最新穩定版）≥ 30、手機（Chrome DevTools Moto G4 throttling profile，10 秒錄製平均值）≥ 24；量測方式與 AC-H03-3 一致。
  - AC-P03-2：Given 其他玩家的路徑揭曉，When 動畫播放，Then 自己的路徑以灰色或淡色呈現，已揭曉玩家的結果在動畫結束後持久顯示於底部結果槽。

- **US-P04**：作為玩家，我想要在結果揭曉後立即知道自己是否中獎，以便確認抽獎結果。
  - AC-P04-1：Given 自己的路徑揭曉完畢，When 玩家路徑抵達底部結果槽，Then 畫面顯示「恭喜中獎！」或「未中獎」明確文字，且結果與伺服器端 `REVEAL_INDEX` payload 中 `result` 欄位一致（QA 驗證方式：截圖比對 UI 文字與 WebSocket 訊息 payload `result` 值）。
...
- FR-01-1：系統須在 2 秒內生成唯一 6 碼 Room Code，字元集為大寫英文字母 A-Z（排除 O、I）加上數字 2-9（排除 0、1），共計 24+8=32 字元集。字元集精確定義：`ABCDEFGHJKLMNPQRSTUVWXYZ23456789`（排除視覺混淆字元 O、I、0、1）。
- FR-01-2：Room Code 在 Redis 中以 TTL = 4 小時儲存，過期後自動釋放；碰撞則重試最多 10 次（依 PDD §7.1）。
- FR-01-3：每個房間對應一份完整狀態物件，包含：roomCode、title（string | null，房間名稱，0～50 字）、status、hostId、players[]、winnerCount、ladderMap、resultSlots、revealIndex、seed、rowCount。
...
- FR-04-1：房間狀態流轉：`waiting` → `running` → `revealing` → `finished` → `waiting`（再玩一局）。狀態轉換觸發：`waiting→running` 由 Host 點擊「開始遊戲」；`running→revealing` 由 Host 送出 `BEGIN_REVEAL`；`revealing→finished` 由 Host 送出 `END_GAME`（所有路徑揭曉後才允許）；`finished→waiting` 由 Host 送出 `PLAY_AGAIN`。
- FR-04-2：每次狀態轉換須廣播對應事件（`ROOM_STATE` 或 `ROOM_STATE_FULL`）給所有連線客戶端。
- FR-04-3：非法狀態轉換由伺服器拒絕並回傳 `INVALID_STATE` 錯誤，不觸發任何廣播。
- FR-04-4：`waiting→running` 由 Host 傳送 `START_GAME` 訊息觸發，Server 以單一原子操作完成 seed 生成、ladderMap 計算、resultSlots 指派及 status 寫入（seed 生成與樓梯計算合一）；`running→revealing` 由 Host 傳送 `BEGIN_REVEAL` 訊息觸發，Server 僅執行狀態轉換；seed 及完整樓梯資料在 status=finished 前禁止傳送給任何客戶端。
...
- FR-06-2：客戶端 → 伺服器操作（WebSocket 訊息或 HTTP REST）：`START_GAME`（主持人開始遊戲）、`UPDATE_WINNER_COUNT`（設定 W）、`BEGIN_REVEAL`（開始揭曉）、`REVEAL_NEXT`（下一位）、`SET_AUTO_REVEAL`（設定自動間隔）、`REVEAL_ALL`（全部揭曉）、`END_GAME`（結束本局）、`PLAY_AGAIN`（再玩一局）、`KICK_PLAYER`（踢除玩家）、`JOIN_ROOM`（加入房間）、`RECONNECT`（重連帶 playerId）。
...
- FR-11-3：主持人等待大廳須顯示「複製邀請連結」按鈕，點擊後以 Clipboard API 將 `{origin}/?room={roomCode}` 寫入剪貼簿；Clipboard API 不可用時顯示可全選的 `<input>` 文字框作為 Fallback。
- FR-11-4：玩家加入頁面須在頁面載入時讀取 `localStorage.getItem('ladder_last_nickname')`，若有值則自動預填暱稱欄位；玩家成功加入房間後（WebSocket 握手成功），系統須將暱稱寫入 `localStorage.setItem('ladder_last_nickname', nickname)`。
...
- 目標：主持人斷線後 60 秒內重連，其他玩家的 WebSocket 連線不中斷，且房間狀態（status、players、revealIndex 等）不發生非預期變更。
- 量測：自動化測試模擬 WebSocket 強制斷線並量測重連與狀態同步時間；另行驗證主持人斷線期間其他客戶端收到的 `ROOM_STATE` 廣播次數為 0（無非預期廣播）。

### NFR-05 安全性
- 目標：所有 Host 操作須驗證 `token`（JWT），非法或過期 token 回傳 401；操作者非 Host 回傳 403。
- 目標：玩家 `playerId` 為 UUID v4（不可猜測）；kickedPlayerIds 機制確保被踢玩家無法在同局重連。
- 目標：WebSocket 訊息大小上限 64KB，超過則拒絕並關閉連線。
- 目標：`seed` 在 status=`finished` 前禁止傳送給任何客戶端（防止預測抽獎結果）。
- 目標：自動揭示間隔 T 非整數或超出 1～30 秒範圍時，回傳 `INVALID_AUTO_REVEAL_INTERVAL` 錯誤。
- 量測：手動滲透測試驗證 JWT 驗證、輸入邊界與黑名單機制。

### NFR-06 前端效能預算
- 目標：首頁 JS bundle（gzip）< 80KB；遊戲頁面 JS bundle（gzip）< 150KB；CSS < 30KB。
- 目標：FCP < 1.5 秒（Lighthouse "Simulated Slow 4G" 節流條件，即下行 1.6Mbps、RTT 150ms）；LCP < 2.5 秒；CLS < 0.1。
...
*PRD 版本：v1.4*  
*生成時間：2026-04-20*  
*基於 PDD v2.2（Ladder Room Online）*
...
- **影響範圍**：§US-H（新增 US-H09 邀請 link）、§US-P01 AC（localStorage 暱稱預填）、§FR-11（補充 FR-11-3/FR-11-4）
- **修正/實作內容**：新增 US-H09（主持人複製邀請連結）、AC-P01-5/AC-P01-6（URL 預填房號 + localStorage 暱稱自動預填）、FR-11-3/FR-11-4
- **commit**：e0d2dda