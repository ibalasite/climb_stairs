...
- **US-H03**：作為主持人，我想要在所有玩家就緒後開始遊戲，以便正式進入揭曉流程。
  - AC-H03-1：Given 房間狀態為 `waiting` 且 N ≥ 2 且 W 已設定，When 主持人點擊「開始遊戲」，Then 狀態轉為 `running`，後端生成 seed（UUID hex，以 djb2 hash 轉 uint32）並廣播 `ROOM_STATE`（status=`running`，含 seed 與 rowCount）；**樓梯結構（ladderMap、resultSlots）於後續 `BEGIN_REVEAL` 時以 seed 原子生成，不在此步驟廣播**。
...
- **US-H04**：作為主持人，我想要以手動逐步揭曉每條路徑，以便配合現場節奏製造懸念。
  - AC-H04-0：Given 房間狀態為 `running`，When 主持人點擊「開始揭曉」（觸發 `BEGIN_REVEAL`），Then 伺服器以單一原子操作完成 ladderMap、resultSlots、所有 startColumn/endColumn/result 及 status（`revealing`）的寫入，並廣播 `ROOM_STATE`（status=`revealing`），所有客戶端顯示揭曉控制介面；操作須在 1 秒內完成。
  - AC-H04-1：Given 狀態為 `revealing`，When 主持人點擊「下一位」，Then 伺服器廣播 `REVEAL_INDEX`（含對應玩家 index），所有客戶端同步播放該玩家的行走動畫。
  - AC-H04-2：Given 所有路徑已逐步揭曉，When 主持人點擊「下一位」，Then 系統不回應（按鈕 disabled），頁面顯示「結束本局」按鈕。
  - AC-H04-4：Given 所有路徑已揭曉且主持人點擊「結束本局」（觸發 `END_GAME`），When 伺服器收到 `END_GAME`，Then 狀態轉為 `finished`，廣播 `ROOM_STATE`（status=`finished`，含完整得獎名單），所有客戶端顯示最終結果頁面；操作須在 1 秒內完成。
  - AC-H04-3：Given 狀態為非 `running`（如 `waiting`、`revealing`、`finished`），When 主持人嘗試觸發 `BEGIN_REVEAL`，Then 伺服器拒絕並回傳 `INVALID_STATE` 錯誤，狀態不變。
...
- **US-H06**：作為主持人，我想要一鍵揭曉全部路徑，以便在時間緊迫時快速完成抽獎。
  ...
  - AC-H06-2：Given `REVEAL_ALL` 廣播後，When 動畫播放完畢（或超時跳幀），Then 狀態自動轉為 `finished`，顯示完整得獎名單。
