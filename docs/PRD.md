...
**版本**：v1.2  **日期**：2026-04-19  **基於 PDD**：v2.2
...
- **US-H01**：作為主持人，我想要建立一個帶有唯一 6 碼房間碼的房間，以便玩家能快速找到並加入我的抽獎活動。
  - AC-H01-1：Given 主持人點擊「建立房間」，When 系統接受請求，Then 於 2 秒內回傳 6 碼 Room Code（由大寫 A-Z 排除 O/I、數字 2-9 組成，共 32 字元集，長度固定 6 碼）且在 Redis 中不重複。
...
- **US-H03**：作為主持人，我想要在所有玩家就緒後開始遊戲，以便正式進入揭曉流程。
  ...
  - AC-H03-3：Given 遊戲開始後，When 任何玩家（含主持人）收到 `ROOM_STATE`，Then 所有客戶端渲染的樓梯結構與橫槓位置完全一致，Canvas FPS 桌機 ≥ 30、手機 ≥ 24（量測：Chrome DevTools Performance，模擬 mid-range 手機 Moto G4 profile，10 秒錄製平均值）。
