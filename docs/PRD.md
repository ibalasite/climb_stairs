...
- **US-H06**：作為主持人，我想要一鍵揭曉全部路徑，以便在時間緊迫時快速完成抽獎。
  ...
  - AC-H06-2：Given `REVEAL_ALL` 廣播後，When 客戶端動畫播放完畢（或超時跳幀），Then 主持人介面自動顯示「結束本局」按鈕（不需再點「下一位」）；主持人點擊後觸發 `END_GAME`，伺服器將狀態轉為 `finished` 並廣播最終 `ROOM_STATE`。**`REVEAL_ALL` 本身不自動觸發 `finished`，仍需主持人確認以維持揭曉節奏控制**。
